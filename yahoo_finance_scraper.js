...
const excelPath = path.join('C:', 'Users', 'billy', 'OneDrive', 'Bureau', 'Finan
ces', 'Investissements', 'Stocks', 'Long', 'IBKR API Python - Stock Info - v2.xl
sx');
const outputFolder = path.join('C:', 'Users', 'billy', 'OneDrive', 'Bureau', 'Fi
nances', 'Investissements', 'Stocks', 'Long', 'XLSX');
...
async function withRetry(fn, description = '') {
  for (let i = 0; i < MAX_RETRIES; i++) {
    try {
      ...
      console.warn(`⚠️ ${description} failed (try ${i + 1}/${MAX_RETRIES}): ${er
r.message}`);
...
async function fetchPriceTarget(ticker) {
  const result = await yahooFinance.quoteSummary(ticker, { modules: ['financialD
ata'] });
...
async function fetchKeyStats(ticker, browser) {
  ...
  try {
    const url = `https://finance.yahoo.com/quote/${ticker}/key-statistics?p=${ti
cker}`;
    await page.goto(url, { waitUntil: 'networkidle2', timeout: 60000 });

    const mainTableSelector = 'table.table.yf-kbx2lo, div[data-test="key-stats-t
able"] table';
...
function writeToExcel(ticker, sheetName, data) {
  ...
  let wb = fs.existsSync(filePath) ? XLSX.readFile(filePath) : XLSX.utils.book_n
ew();
